...
impl TCPReceiver {
    fn receive(&mut self, message: TCPSenderMessage,
        ...
        if message.syn || self.syn {
            ...
            reassembler.insert(first_byte.unwrap(&self.start,
                                                 inbound_stream.bytes_pushed()),
            message.payload,
            message.fin,
            inbound_stream);
            self.ackno = Some(Wrap32::wrap(inbound_stream.bytes_pushed(), &self.start) +
                inbound_stream.is_closed() as u32)
        }
    }

    fn send(&self, inbound_stream: &impl Writer) -> TCPReceiverMessage {
        TCPReceiverMessage::new(self.ackno.clone(),
                                min(u16::MAX as usize,
                                    inbound_stream.available_capacity()) as u16)
    }
}